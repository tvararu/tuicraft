...
`spirit-healer <guid>` requests resurrection from one observed creature whose NPC flags carry the healer bit (0x4000). It requires observed ghost state, rejects an unanswered duplicate request, never auto-activates, and never reports success on intent. Server spirit resurrection may incur durability loss. Gossip option 0 stays silent. After `OK`, inspect `recovery --json` for observed `life=alive`.
