...
:: List nearby entities ordered by raw 3D distance from the current `control.pose`, before display rounding. When no control pose exists, the last self-entity position is a fallback. By default, entities beyond 100 yards, on another map, or without a world position are filtered when self position is known; `--all` lists all tracked entities, including map-wide transports and the character's carried items and bags, which have no world position and so have `null` coordinates, distances and angles. The 100-yard limit derives from `DEFAULT_VISIBILITY_DISTANCE 100.0f` in AzerothCore's `src/server/game/Entities/Object/ObjectDefines.h:39` ('100 yards on continents'). Because `Map::GetVisibilityRange()` is per-map configurable (e.g. 170 yards in instances, 250 yards in battlegrounds and arenas), 100 yards is a continent default rather than universal across all maps.
...
Non-self distances and angles are `null` if either position is unknown or off-map. `originSource` is `predicted`, `server`, `self_entity`, or `null`. `originUpdatedAt` is the control pose update time, or `null` for fallback. Other entity positions are the latest observations, not guaranteed current. `mapId` is the map at entity parsing, not a server field per entity. Straight-line distance and bearing do not prove a safe or reachable route.
Rows with a position also carry `positionSource`, `positionKind`, `positionObservedAt` and `positionAgeMs`. `positionSource` is `update_object` (spawn or object update), `movement` (a player's `MSG_MOVE_*` broadcast), `monster_move` (`SMSG_MONSTER_MOVE`, also inside `SMSG_COMPRESSED_MOVES`), or `control` for the self row's control pose. `positionKind` is `observed` for a position the server sent and `predicted` for one sampled along the last monster-move path, the same position the `tactics` observation and `walk-toward` use; a finished path stays `predicted` at its end until the next observation, and text output adds `(predicted)` after such a position. `positionObservedAt` is the local ms time of that observation (or `null` when unknown) and `positionAgeMs` is the time since. A creature that ran to you and died is listed where it stopped.
...
running `fight` or `cycle` like other manual actions, but the way a Jev failure
