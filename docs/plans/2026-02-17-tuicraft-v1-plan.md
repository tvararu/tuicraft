# tuicraft v1 Implementation Plan
...
export class PacketReader {
  ...
  constructor(private data: Uint8Array, offset = 0) {
...
test("SRP computes session key from known parameters", () => {
  ...
  const a = BigInt("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
...
test("SRP session key K is 40 bytes with interleaved hashing", () => {
  ...
  const a = BigInt("0xabcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789")
...
import { test, expect } from "bun:test"
import { buildLogonChallenge, parseLogonChallengeResponse, parseRealmList } from "./auth"
...
test("parseRealmList extracts realm info", () => {
  const w = makeRealmListPacket([
    { icon: 0, lock: 0, flags: 0, name: "Test", address: "127.0.0.1:8085", population: 1, characters: 2, timezone: 1, id: 1 },
  ])
...
function makeRealmListPacket(realms: Array<{
  icon: number; lock: number; flags: number; name: string;
  address: string; population: number; characters: number;
  timezone: number; id: number;
}>): Uint8Array {
  const parts: number[] = []
...
import { SRP, leBytesToBigInt, bigIntToLeBytes, type SRPResult } from "../crypto/srp"
...
test("OpcodeDispatch persistent handler fires on matching opcode", () => {
  ...
  dispatch.on(0x1234, () => { received = true })
...
test("OpcodeDispatch expect takes priority over persistent handler", async () => {
  ...
  dispatch.on(0x1111, () => { persistentCalled = true })
...
function compressAddonInfo(): Uint8Array {
  ...
  for (const addon of ADDONS) {
    for (let i = 0; i < addon.name.length; i++) parts.push(addon.name.charCodeAt(i))
    parts.push(0)
...
export function decryptIncomingHeader(header: Uint8Array, arc4?: Arc4): { size: number; opcode: number } {
...
if (!values.account || !values.password || !values.character) {
  console.error("Usage: bun src/index.ts --account <account> --password <password> --character <name>")
  process.exit(1)
}
...
async function authHandshake(): Promise<{ sessionKey: Uint8Array; realmHost: string; realmPort: number; realmId: number }> {
        ...
        data(socket, data) {
          ...
          if (state === "challenge") {
            ...
            if (challenge.status !== 0x00) {
              reject(new Error(`Auth challenge failed: status 0x${challenge.status.toString(16)}`))
              socket.end()
              return
            }
            ...
            srpResult = srp.calculate(challenge.g, challenge.N, challenge.salt, challenge.B)
            ...
            if (proof.status !== 0x00) {
              reject(new Error(`Auth proof failed: status 0x${proof.status.toString(16)}`))
              socket.end()
              return
            }
...
async function worldSession(sessionKey: Uint8Array, realmHost: string, realmPort: number, realmId: number) {
  const dispatch = new OpcodeDispatch()
  const connectionTime = Date.now()
  let arc4: Arc4 | undefined
  let worldSocket: ReturnType<typeof Bun.connect> extends Promise<infer T> ? T : never
  ...
  dispatch.on(GameOpcode.SMSG_TIME_SYNC_REQ, (r) => {
    ...
    w.uint32LE(counter)
    w.uint32LE(elapsed)
    worldSocket.write(buildOutgoingPacket(GameOpcode.CMSG_TIME_SYNC_RESP, w.finish(), arc4))
  })
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    Bun.connect({
      ...
      const challengeReader = await dispatch.expect(GameOpcode.SMSG_AUTH_CHALLENGE)
      challengeReader.uint32LE()
      const serverSeed = challengeReader.bytes(4)

      console.log("World auth challenge received, sending proof...")
      const authBody = await buildWorldAuthPacket(ACCOUNT, sessionKey, serverSeed, realmId)
      ...
      socket.write(buildOutgoingPacket(GameOpcode.CMSG_CHAR_ENUM, enumBody, arc4))

      const charReader = await dispatch.expect(GameOpcode.SMSG_CHAR_ENUM)
      const characters = parseCharacterList(charReader)
      console.log(`Characters: ${characters.map(c => `${c.name} (Lv${c.level})`).join(", ")}`)
      ...
      socket.write(buildOutgoingPacket(GameOpcode.CMSG_PLAYER_LOGIN, loginW.finish(), arc4))
      ...
      setInterval(() => {
        ...
        socket.write(buildOutgoingPacket(GameOpcode.CMSG_PING, w.finish(), arc4))
      }, 30000)
    })
  })
}
