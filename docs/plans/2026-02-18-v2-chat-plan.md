...
**Reference:** `../wow-chat-client` for packet formats. `docs/plans/2026-02-18-v2-chat-design.md` for full design.
...
### Task 7: Live Integration Testing

**Files:**

- Modify: `src/test/live.ts`

Test against the real server with two clients. One whispers the other, say messages, /who queries.

**Step 1: Write two-client live tests**

Replace `src/test/live.ts`:
...
Run: `mise test:live`
Expected: All live tests PASS. The whisper test confirms name resolution works end-to-end.

**Step 3: Debug any protocol issues**

If tests fail, cross-reference `../wow-chat-client` for correct packet formats. Common issues:

- SMSG_MESSAGE_CHAT format may differ slightly for GM messages
- Name query packed GUID encoding
- Chat message length field (bytes, not including null terminator)

Fix issues, re-run `mise test:live` until all pass.
