...
- Eat and drink out of combat; potions work in combat. Sending the use stops a running `fight` or `cycle` (manual override; a refused `use` does not) the way a Jev failure stop does: fight movement stops, auto-attack stays on a live target attacking you (`tactics --json` `defense: auto_attack`), so re-run `fight` afterwards to hand it back to Jev. Outside a fight the use leaves movement and auto-attack alone. Moving stands you up and ends food and drink.
