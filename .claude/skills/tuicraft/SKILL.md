...
- `navigation --json` retains `blockedReason` and `refusal` and adds `nextStep`; a `pick_destination` refusal also has `floors` (highest first). After `obstructed`, choose another route. After `height_unresolved`, try a different short heading or known grounded waypoint. After `ambiguous ground column at destination` or `destination is not on a ground floor`, repeat the `goto` with one of `floors` as Z or choose another destination. `ambiguous ground column at start` (a surface less than 1.6 yards above the pose) needs a move to open ground first, and so does `leaving start`; do not change the destination for either. `at route` needs another destination or waypoint. `refusal=unreachable` means the navigation mesh cannot reach that destination; choose another one and never retry it. After `pathfind_find_height failed (UNKNOWN_HEIGHT)` (`refusal=stop`), do not repeat the `goto` unchanged: move about 10 yards off the spot with `face` and `move forward`, then plan again, or choose a nearer grounded waypoint; when several destinations fail this way from one pose, move first. Only `navigation` gives that advice; `control` and CONTROL events keep `nextStep: null` for this reason because a mid-walk stop with it replans itself. Do not guess Z or repeat an unsafe heading. No hint proves the next route safe.
