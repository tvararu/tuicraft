...
- `halt` stops walking, casting, auto-attack, tactics, navigation, and cycle, and clears an unanswered spirit-healer request. On one IPC socket it interrupts pending work and drops older queued mutating control, recovery, quest, and loot commands, including `cycle`, plus older read waits. It retains state inspections, corpse and metadata queries, and newer requests. It cannot undo a sent request or disengage an attacking enemy. `status` remains CONNECTED.
...
- `fight` refuses a GUID with no entity in view (never seen, or left view) with `target_unobserved`, and `cycle` skips it with that cause. It does not mean the GUID is not a creature: move closer or re-read `nearby`.
- `no_progress` blocks a fight after 30 s in which the target's health never fell below its lowest observed value and the separation never closed at least 1 yd below its closest value (an unobserved separation never counts). Casts, shields, heals and waits alone are not progress; `cycle` skips the target with cause `no_progress`.
- `target_unreachable` blocks a fight when the target stays out of melee and out of range of every supported spell for 5 s in which the separation never closes at least 1 yd below its closest value since it went out of reach. A character still walking in is not stopped; coming back into range resets the bound. `cycle` skips the target with cause `target_unreachable`.
...
- A recoverable server rejection (not in front 61/134 or `bad_facing`, out of range 97 or `not_in_range`, too close 128, line of sight 47, moving 51, not standing 69, not ready 67, in progress 105, 173, no power 85, interrupts 10/40/41 or an interrupted status, stunned 108, silenced 104, pacified 98, confused 26, fleeing 34) does not end a fight. The observation shows it as `rejections` (`consecutive`, `limit`, `last`), and a facing rejection offers `face_target`. Three in a row without a started or successful action stop with `blocked`/`server_action_rejected:<reason>`; any other rejection stops at once. Both stops set `defense` like a Jev failure stop.
...
- A Jev request over 5 s is discarded as `jev_timeout` (no action) and the loop asks again. Only 3 timeouts in a row stop the run (`failed`/`jev_timeout`); an answered request resets the count. Other Jev failures stop at once, except the network failures below. `tactics --json` shows `timeouts` (`consecutive`, `total`, `limit`) and, after a Jev failure or server rejection stop, `defense`: `auto_attack` (kept or started on a live target attacking you), `uncontrolled_in_combat` (released while still in combat: act yourself), or `none`.
...
- `cycle --quest <id>` states only the objective: the daemon derives the objective creatures from the server quest query, picks the nearest live, untried, untapped needed creature within 50 yd, lets Jev fight it, loots, re-reads the quest log and repeats. It stops with `objective_complete` when the log slot is complete. On `objective_targets_absent` or `objective_targets_out_of_reach` (`stopDetail` has the nearest GUID and distance), travel closer and start it again or `cycle --resume` (tried creatures are never re-picked). Item objectives need `--source <entry>` for each creature that drops the item (the quest query does not name drop sources). GameObject objectives are unsupported. `--max` defaults to twice the required count. `cycling --json` `objective` shows the server counters. Accepting, travel and turn-in remain explicit steps.
...
- Cycle loot keeps one carried bag slot free for a quest reward. Items that fit into a carried stack (stack size from the item query) need no slot. Items a logged quest requires (quests whose query is known; the daemon queries every logged quest itself) are taken first. Another item that would need the last free slot stays in the corpse and its loot slot is listed in `lastLoot.slotsLeft`; a quest item that would need it closes the window and stops with `inventory_reserve_reached` (`stopDetail`: `itemId`, `lootSlot`, `freeSlots`, `reserve`). Then `destroy` or `sell` a stack and `cycle --resume` (the next target; the item stays in that corpse for `open-loot`). Unobserved free slots allow only items that fit into carried stacks.
- Jev unavailability is not a target failure. A refused key (HTTP 401/402/403, cause `HTTP <status> <error_type>`, for example `HTTP 402 billing_error` when TypeSafe has no credits), three consecutive network failures, rate limits or server errors (`transport <error> (3 in a row)`), or a missing key (`missing_jev_key`) makes `fight` reply `ERR jev_unavailable: <cause>` (exit 1, error envelope with `--json`). A cycle stops with `stopCause` `jev_unavailable` and `stopDetail.reason`, keeps the current and later targets `queued`, and `cycle`/`cycle --resume` reply the same `ERR`. Do not retry fights while it persists; `cycle --resume` once Jev is back refights the same target. One network failure is retried in-fight; `jev_timeout` is separate.
- Inspect `cycling --json` for `phase`, per-target `queue` status/cause/loot, `instruction`, `startsUsed`, `resumes`, `lastRecovery`, `stopCause`, `stopDetail`, and `lastLoot`. `stopCause` is an open string: examples are `queue_exhausted`, `max_starts_reached`, `halt`, `target_death_unconfirmed`, `loot_denied:*` (including `loot_denied:timeout` for an unanswered take), `loot_inventory_full`, `inventory_reserve_reached`, `loot_denied:release_only`, `loot_release_unconfirmed`, and recovery causes. The loop waits for the server release acknowledgement after close before recording loot. A stop such as `loot_inventory_full` leaves the loot window open for `destroy` and `take-loot`; the next corpse a cycle loots first releases a window that is still open. Inspect `stopDetail`.
- `lastLoot.slotsTaken` records requested slots, `slotsLeft` the slots left in the corpse to keep a bag slot free, `moneyTaken` records offered money, and before/after coinage is observed when known. None of these proves item storage. Check raw inventory slot/count changes before claiming a gain.
...
- `goto` takes two or three finite coordinates. It is not a named-place planner. A floor is a native column surface with no other surface 0.25 to 1.6 yards above it. Without Z, the daemon takes the destination height from the one floor there; a column with several floors refuses with `ambiguous ground column at destination (floors …)` and `refusal=pick_destination` before any movement. With Z, it must be within 0.25 yards of a floor: repeat with one of the listed floors to pick it. A Z on no floor refuses with `destination is not on a ground floor (floors …)`. The route must still reach the chosen floor over connected ground. Prefer the two-coordinate form over copying `nearby` Z. A new `goto` during an active route redirects it: the old route stops with CONTROL `movement_stopped` reason `navigation_replaced` and the new route plans from the stopped pose. A refused redirect leaves the character stopped.
- `goto` needs the start pose on a floor of its column (a platform or upper level is fine). Along the route it accepts surfaces overhead (more than 1.6 yards up) or below, and refuses surfaces within 1.6 yards above and steps over 1 yard (`ambiguous ground column at route`). From a start column with several floors (a platform or building), that refusal before the route reaches open ground (a column with one floor) is `ambiguous ground column leaving start`: the start, not the destination, is the problem.
- `goto <guid>` plans once to the ground under an observed creature and does not follow it. Over a column with several floors it picks the one floor within 0.25 yards of the creature's observed Z, so a creature on a terrace or under a platform needs no Z; with no such floor, or two, it refuses at `pick_destination` like the coordinate form. If the creature disappears (despawn or out of range) during the route, it stops with `blockedReason=target_lost`; nothing replans. Pick a current target from `nearby --json` before trying again.
- A route stopped mid-walk by a ground refusal or a server correction replans itself from the stopped pose to the same destination, within limits. While `navigation --json` shows `replan.pending=true`, wait; do not issue another `goto`. Limits: at least 2 yd displacement since the previous plan, at most 4 plans, 60 s and `replan.limits.traveled` yards. A limit stops with `replan_no_progress`, `replan_plan_limit`, `replan_time_limit` or `replan_distance_limit`; a refused replan stops with `replan_refused: <planner reason>` (for example UNKNOWN_HEIGHT). These stops are terminal: choose another destination instead of repeating the same `goto`.
...
- `navigation --json` retains `blockedReason` and `refusal` and adds `nextStep`; a `pick_destination` refusal also has `floors` (highest first). After `obstructed`, choose another route. After `height_unresolved`, try a different short heading or known grounded waypoint. After `ambiguous ground column at destination` or `destination is not on a ground floor`, repeat the `goto` with one of `floors` as Z or choose another destination. `ambiguous ground column at start` (a surface less than 1.6 yards above the pose) needs a move to open ground first, and so does `leaving start`; do not change the destination for either. `at route` needs another destination or waypoint. After `start snapped off the requested ground position` the pose is off the walkable mesh, next to an object or a building: move 3 to 5 yards into open ground with `face` and `move forward`, then plan again; do not repeat the `goto` from there. After `ground corridor changes surface`, choose a nearer waypoint on the same floor or another destination. `refusal=unreachable` means the navigation mesh cannot reach that destination; choose another one and never retry it. After `pathfind_find_height failed (UNKNOWN_HEIGHT)` (`refusal=stop`), do not repeat the `goto` unchanged: move about 10 yards off the spot with `face` and `move forward`, then plan again, or choose a nearer grounded waypoint; when several destinations fail this way from one pose, move first. Only `navigation` gives that advice; `control` and CONTROL events keep `nextStep: null` for this reason because a mid-walk stop with it replans itself. Do not guess Z or repeat an unsafe heading. No hint proves the next route safe.
...
- Auto-accept quests enter the log on `select-quest` while their details stay open. `quests` marks their details `(auto-accept)` (`--json`: `dialog.data.autoAccept`). Check the log first: `accept-quest` fails with `quest_already_in_log <id>` and sends nothing for a quest already there. A server refusal sets `lastError: {kind: "invalid", reason: 13, reasonName: "already_on_quest"}`.
...
- Accept: `talk <giver>`, `select-quest <id>` (details), `accept-quest`, then confirm the log. Turn in: `talk <giver>`, `select-quest <id>`. An item quest (8326) opens `requestItems`, then `request-reward` opens `offer`. A kill or talk quest (8325, 8564) opens `offer` directly. Finish with `choose-reward <index>`.
- Check the log before `accept-quest`: many quests (8325, 8326) are accepted as soon as `select-quest` opens `details`; close those with `cancel-interaction`. After a reward the giver may open the next quest's `details` (8326 opens 8327, already accepted). Item objectives show carried counts from the inventory (`items`).
- `complete-quest <id>` sends the same complete request that `select-quest` sends for a quest you can turn in. On a `requestItems` dialog it only re-sends that dialog with no error, so use `request-reward` there. `request-reward` is the Continue button: the server completes the quest if the required items and money are present and sends `offer`; it sends nothing otherwise.
...
- Item objectives stay 0 in the log counters. The daemon queries every quest in the log (at login and on accept), so after a restart or relog `quests --json` `items` (required vs carried) is still filled without `query-quest`. Watch QUEST `progress` events with source `inventory` (`lastProgress.kind` `collect`: item push total matched to the observed bags). `itemPushes` holds pushes the bags have not caught up with. Completion is still the log slot flag.
...
- Inventory-full/bag-full errors retain their raw result/details. The server refuses a take it cannot store only with such an error, so the error settles the pending take (`pending` clears) and the item stays offered. Free a slot (`destroy`) and `take-loot` the same slot again from the open window.
...
- Eat and drink out of combat; potions work in combat. Sending the use stops a running `fight` or `cycle` (manual override; a refused `use` does not) the way a Jev failure stop does: fight movement stops, a spell the fight was casting is cancelled so the item goes through, and auto-attack stays on a live target attacking you (`tactics --json` `defense: auto_attack`), so re-run `fight` afterwards to hand it back to Jev. Outside a fight the use leaves movement, casts and auto-attack alone. Moving stands you up and ends food and drink.
...
- `buy` takes the vendor slot and the number of purchases (default 1, max 255); `request.count` and the `Last: buy 2 purchases of item 159 ...` line count purchases, each giving `buyCount` items. The server rounds the discounted cost of the whole buy down once, so a multi-purchase buy can cost up to `count - 1` copper more than `price * count`: budget with `request.maxPrice` (text `for 69-71 copper`); `request.minPrice` is `price * count`. `repair` repairs every damaged carried and equipped item and needs a vendor with the repair flag.
