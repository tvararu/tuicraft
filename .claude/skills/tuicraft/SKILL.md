...
- A Jev request over 5 s is discarded as `jev_timeout` (no action) and the loop asks again. Only 3 timeouts in a row stop the run (`failed`/`jev_timeout`); an answered request resets the count. Other Jev failures stop at once. `tactics --json` shows `timeouts` (`consecutive`, `total`, `limit`) and, after a Jev failure or server rejection stop, `defense`: `auto_attack` (kept or started on a live target attacking you), `uncontrolled_in_combat` (released while still in combat: act yourself), or `none`.
