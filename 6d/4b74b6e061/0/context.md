...
Coverage is NOT 100%. Regressions in: commands.ts (99.79% lines), server.ts (96.43% funcs), mock-handle.ts (90% funcs, 98.59% lines), world-handlers.ts (99.81% lines). On main all these were 100%. Find the uncovered lines/functions introduced by the ignore list changes and add tests. Must reach 100% on all files. Commit and force-push.
