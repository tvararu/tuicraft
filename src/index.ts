...
if (!values.account || !values.password || !values.character) {
  console.error("Usage: bun src/index.ts --account <account> --password <password> --character <name> [--host <host>] [--port <port>]")
  process.exit(1)
}
        ...
        data(socket, data) {
          ...
          if (state === "challenge") {
            ...
            if (result.status !== 0x00) {
              socket.end()
              reject(new Error(`Auth challenge failed: status 0x${result.status.toString(16)}`))
              return
            }
            srpResult = srp.calculate(result.g!, result.N!, result.salt!, result.B!)
            ...
            if (result.status !== 0x00) {
              socket.end()
              reject(new Error(`Auth proof failed: status 0x${result.status.toString(16)}`))
              return
            }
...
function worldSession(auth: AuthResult): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    dispatch.on(GameOpcode.SMSG_TIME_SYNC_REQ, (r) => {
      ...
      w.uint32LE(counter)
      w.uint32LE(elapsed)
      socket.write(buildOutgoingPacket(GameOpcode.CMSG_TIME_SYNC_RESP, w.finish(), arc4))
    })
    ...
    async function login() {
      ...
      const body = await buildWorldAuthPacket(account, auth.sessionKey, serverSeed, auth.realmId)
      ...
      if (authStatus !== 0x0c) {
        throw new Error(`World auth failed: status 0x${authStatus.toString(16)}`)
      }

      socket.write(buildOutgoingPacket(GameOpcode.CMSG_CHAR_ENUM, new Uint8Array(0), arc4))

      const enumReader = await dispatch.expect(GameOpcode.SMSG_CHAR_ENUM)
      const chars = parseCharacterList(enumReader)
      const char = chars.find((c) => c.name.toLowerCase() === character.toLowerCase())
      if (!char) {
        throw new Error(`Character "${character}" not found. Available: ${chars.map((c) => c.name).join(", ")}`)
      }

      const loginBody = new PacketWriter()
      loginBody.uint32LE(char.guidLow)
      loginBody.uint32LE(char.guidHigh)
      socket.write(buildOutgoingPacket(GameOpcode.CMSG_PLAYER_LOGIN, loginBody.finish(), arc4))
